...
from qaoa_utils import hamiltonian_from_ising, provide_random_maxcut_QUBO, hamiltonian_from_qubo
...
def build_hamiltonian_ising(J, h, fraction, num_qubits):
    cost_hamiltonian, offset = hamiltonian_from_ising(J, h, 0)
    mixer_hamiltonian = build_mixing_hamiltonian(num_qubits)
    offset_hamiltonian = SparsePauliOp(["I"])
    for i in range(num_qubits - 1):
        offset_hamiltonian = offset_hamiltonian.tensor(Pauli("I"))
...
def calculate_spectral_gap_ising(fraction: float, J: np.ndarray, h: np.ndarray, num_dec_pos: int=4):
    num_qubits = len(J)
    H = build_hamiltonian_ising(J, h, fraction, num_qubits)
    counter = 0
    eigenvalues = []
    eigenstates = []
    while len(set(eigenvalues)) < 2 and counter != num_qubits:
...
def main(
    ...
    for csv_data in csv_data_list:
        save_to_csv(csv_data, result_path_prefix, "spectral_gap_evolution_until_10qubits.csv")
